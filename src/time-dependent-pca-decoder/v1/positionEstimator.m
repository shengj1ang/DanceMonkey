...
% -> regression conditioned on direction and time window
    ...
    % ---------------------------------------
    if isempty(modelParameters.cachedTrialId) || modelParameters.cachedTrialId ~= test_data.trialId
        modelParameters.cachedTrialId = test_data.trialId;
        modelParameters.cachedDirection = classifyDirection(rate, tIdx, modelParameters);
    else
        if currBinCount <= modelParameters.maxBinCount
            modelParameters.cachedDirection = classifyDirection(rate, tIdx, modelParameters);
        end
    end

    dirHat = modelParameters.cachedDirection;
    ...
    % small stabilization using previous decoded point if available
    if isfield(test_data, 'decodedHandPos') && ~isempty(test_data.decodedHandPos)
        prev = test_data.decodedHandPos(:, end);
        x = 0.85 * x + 0.15 * prev(1);
        y = 0.85 * y + 0.15 * prev(2);
    end
end
