...
%
% Method #2:
%   1) Direction classification using cosine similarity to templates built
%      from the first 320 ms (fast and typically robust).
%   2) Per-direction ridge regression to predict velocity (vx, vy) from
%      lag-stacked binned spike counts.
%   3) Feature standardization (mean/std) per direction to improve stability.
%
% Caching:
%   Loads modelParameters.mat if it matches the required fields;
%   otherwise retrains and overwrites the cache.
%==========================================================================
    ...
    if exist(cacheFile, 'file') == 2
        ...
        if isfield(S, 'modelParameters')
            ...
            requiredFields = {'binSizeMs','lagBins','dt','lambda', ...
                              'dirTemplatesUnit','velW','velB','muX','sigX','x0y0'};
            ok = true;
    ...
    for tr = 1:nTrials
        for d = 1:numDirs
            ...
            t320 = min(320, T);
            fr320 = mean(spikes(:, 1:t320), 2); % 98 x 1 (mean spikes per ms)
    ...
    % Unit-normalise templates for cosine similarity
    ...
    % Standardize features per direction: (X - mu) ./ sigma
    velW = cell(1, numDirs);
    velB = cell(1, numDirs);
    muX  = cell(1, numDirs);
    sigX = cell(1, numDirs);
    ...
    for d = 1:numDirs
        ...
        mu = mean(X, 1);
        sg = std(X, 0, 1);
        sg(sg < 1e-6) = 1; % avoid division by zero
    ...
    modelParameters.dirTemplatesUnit = dirTemplatesUnit; % 98 x 8
