...
Spike trains were first converted into 20\,ms bins, followed by a square-root transform and exponential moving average (EMA) smoothing to reduce noise. Features from several recent time windows were concatenated to capture short-term temporal dynamics. Principal component analysis (PCA) was then applied to obtain a low-dimensional representation,
\begin{equation}
\tilde{x} = U_r^{T}(x-\mu),
\label{eq:pca}
\end{equation}
...
\end{equation}
where $D$ is the original feature dimension. In this work, the threshold was set to $0.95$, allowing the representation to adapt to the intrinsic structure of the data while avoiding unnecessary high-dimensional noise.
...
\label{eq:ensemble}
\end{equation}
which reduced sensitivity to early misclassification. Second, predictions were blended with direction-specific mean trajectories, with the influence of the mean decaying over time. Third, displacement clipping was applied to suppress implausible jumps. Finally, lightweight causal smoothing was used to reduce high-frequency jitter.
